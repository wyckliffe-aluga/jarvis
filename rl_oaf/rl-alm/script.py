...
# load a list of depositor and borrower 
...
# Helper functions for pricing value simulation 

# monte carlo simulation for pricing acceptance of clients 
...
# Deposit amount's won't change 
# Depositor and borrower in this case are different parties 
# The depositor class simulates the behavior of one depositor 

class Depositor : 

    def __init__(self, curr_amt, curr_price, curr_m, start_date) : 
        self.curr_amt = curr_amt
        self.curr_price = curr_price
        self.curr_m = curr_m
        self.prob_attrite = random.uniform(0 , 1)
        self.start_dte = start_date
        self.m_dte = self.start_dte + datetime.timedelta(days=self.curr_m)
        self.exist = False
        self.attribute_threshold = curr_price
        return
    
    # set deposit expectations given the market price 
    ...
    # make a carry forward or not decision based on the expection and pricing offered 
    # run at maturity date to determine if the deposit should stay or withdrawn 
    ...
    # generate the amount to pay to the depositor as expense 
...
## BORROWER 
# class borrower simulated the behavior of one borrower 

class Borrower: 

    def __init__(self, curr_amt, curr_price, curr_m, start_dte) : 
        self.curr_amt = curr_amt
        self.curr_price = curr_price
        self.curr_m = curr_m
        self.prob_attrite = random.uniform(0, 1)
        ...
        self.m_dte = self.start_dte + datetime.timedelta(days=self.curr_m)
        self.exist = False 
        self.attrite_threshold = curr_price
        return 
    
    # set loan expectation given the market pricing 
    # only update the expectation if it is maturing day 
    ...
    # make the roll over or not decision baseed on the expectation and pricing offered 
    # run at maturity date to determine if there is any need for refinancing (refinancing the same loan after maturity)
    # expectation will only be updated whe it is maturing today 
    ...
    # generate the amount OAF has to lend to the borrower as income 
    # run any day before or at maturity day --- to generate interest income 
...
class OAF : 
    ...
    def __init__(self, dep_model_path=' ', loan_model_path=' ') : 
        ...
        # create a new deposit model if no file to an existing model is given 
        if dep_model_path == ' ': 
    ...
    # generate pricing grids 
    # these are static pricing grids for loan and deposit 

    # static deposit pricing grid 
    ...
    # update market pricing for market and OAF 
    # update market pricing for markert : pricing grid update according to weiner process 
    ...
    # update market pricing for OAF using ml methode to generate the grid 
    # depost : run predictions for deposit 
    ...
    def ML_prep_x_np(self,maturing_amt, mkt_grid,day_cnt,self_funding_target):
        ...
        for key, value in mkt_grid.items():
            x_np.append(value)
        x_np = np.array(x_np,dtype=float).reshape(1,36)
        return x_np
    ...
    # feedback: feed training set as reinforcement for both models 
    ...
    # generate pricing grids based on existing grid 
...
# environment object will provide reward estimates
# it will generate 1. the net profit of the loan and pricing grids 
# 2. self funding ratio 

class Environment: 

    def __init__(self,reward_model_input_features,reward_model_path ='',fund_model_path=''):

        # create a new deposit model if no file path to an existing model if given 
    ...
    # calculate the amount of deposit up for repricing 
    def save_models(self,reward_path,funding_path):
        self.reward_model.save(reward_path)
        self.reward_model.save(funding_path)
        return True
# load the list of loans /depositors and the list of borrowes as list of objects 
...
for run in range(0,12):
    ...
    #oaf  and environment objects created
    if run==0:
        jpm = OAF()
        env = Environment(reward_model_input_features)
    else:
        jpm = OAF(dep_model_path, loan_model_path)
        prev_failed_bank = False
